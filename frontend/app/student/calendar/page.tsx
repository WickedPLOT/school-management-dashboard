...
export default function StudentCalendarPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      try {
        ...
        const schedules = await apiFetch('/student/daily-schedule');

        // Get events
        const eventsList = await apiFetch('/student/attendance/events');
  ...
  return (
    <div className="section-shell">
      <div className="page-header">
        <h1>Calendar</h1>
        <p>View all activities and events for your section</p>
      </div>
      ...
      {events.length === 0 ? (
        <div className="section-outline" style={{ padding: '3rem 1rem', textAlign: 'center' }}>
          <div style={{ color: 'var(--muted)' }}>
            <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>📅</div>
            <p>No upcoming activities or events scheduled</p>
          </div>
        </div>
      ) : (
